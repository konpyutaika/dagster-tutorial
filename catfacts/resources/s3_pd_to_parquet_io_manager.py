...
class S3PandasToParquetIOManager(IOManager):
    def __init__(
            self,
            s3_bucket,
            s3_session,
    ...
    def load_input(self, context: "InputContext") -> Optional[pd.DataFrame]:
        ...
        obj = self.s3.get_object(Bucket=self.s3_bucket, Key=key)["Body"].read()
        parquet_buffer = BytesIO(obj)
        return pq.read_table(parquet_buffer).to_pandas()
    ...
    def handle_output(self, context: "OutputContext", obj: pd.DataFrame) -> None:
        ...
        self.s3.put_object(Body=parquet_buffer.getvalue(), Bucket=self.s3_bucket, Key=key)
...
def s3_pd_to_parquet_io_manager(init_context):
    ...
    return S3PandasToParquetIOManager(s3_bucket, s3_session, s3_prefix=s3_prefix)
